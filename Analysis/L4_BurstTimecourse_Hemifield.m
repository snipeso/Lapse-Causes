...
ProbBurst_Stim = nan(numel(Participants), 2, 3, 123, 2, size(Windows_Stim, 1)); % P x VHF x TT x Ch x B x W
ProbBurst_Resp = nan(numel(Participants), 2, 3, 123, 2, size(Windows_Resp, 1));
        ...
        for Indx_Ch = 1:TotChannels
            for Indx_TT = 1:3
                ...
                TT_Indexes = AllTrials_Table.Type==Indx_TT & ...
                    ...
                    AllTrials_Table.isRight == isRight(Indx_VHF);
                ...
                ProbBurst_Stim(Indx_P, Indx_VHF, Indx_TT, Indx_Ch, Indx_B, :) = ...
                ...
                if Indx_TT>1 % not lapses
                    ...
                    ProbBurst_Resp(Indx_P, Indx_VHF, Indx_TT, Indx_Ch, Indx_B, :) = ...
...
for Indx_P = 1:numel(Participants)
    for Indx_B = 1:numel(BandLabels)
        if any(isnan(ProbBurst_Stim(Indx_P, :, :, :, Indx_B, :)), 'all')  % P x VHF x TT x Ch x B x W
...
%%% save
t = t_window;
save(fullfile(Pool, 'ProbBurst_Hemifield.mat'), 'ProbBurst_Stim', 'ProbBurst_Resp', 't', 'GenProbBurst', 'Chanlocs')
