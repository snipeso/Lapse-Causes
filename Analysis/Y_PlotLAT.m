...
[Lapses, ~] = tabulateTable(Trials(Trials.EC==0 & Trials.Type==1, :), 'Radius_Bins', 'tabulate', Participants, Sessions, SessionGroups);
[Tots, ~] = tabulateTable(Trials(Trials.EC==0, :), 'Radius_Bins', 'tabulate', Participants, Sessions, SessionGroups);

Lapses = 100*Lapses./Tots;
Lapses = permute(Lapses, [1 3 2]);

subfigure([], Grid, [1 3], [1 1], true, PlotProps.Indexes.Letters{3}, PlotProps);
plotSpikeBalls(Lapses, [], {'BL', 'SD'}, [getColors(1, '', 'gray'); PlotProps.Color.Types(1, :)], PlotProps)
ylabel('% lapses')
ylim([0 80])
